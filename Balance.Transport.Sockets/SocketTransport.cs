...
using Balance.Transport.Abstractions;
using Banace.Hosting.Abstractions;
...
namespace Balance.Transport.Sockets
{
	internal sealed class SocketTransport : ITransport
	{
		...
		private async Task HandleConnectionAsync(SocketConnection connection)
		{
			try
			{
				Task middlewareTask = _dispatcher.OnConnection(connection);
				Task transportTask = connection.StartAsync();
